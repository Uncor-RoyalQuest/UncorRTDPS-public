...
namespace UncorRTDPS.UncorRTDPS_Windows.DpsPanel
{
    public class Optimizer_DamageInfoSequence
    {
        ...
        /// </summary>
        /// <param name="dt_dm"></param>
        public void UpdateDamageInfo(DamageTarget_DamageModel dt_dm)
        {
            ...
            panel_DamageInfos[0].UpdateViewData(dt_dm, dt_dm.target);
        ...
        public void UpdateDamageInfo(DamageTarget_DamageModel dt_dm, int posAt)
        {
            //create new if needed, therefore provide confident access to [0] element 
            FillListPanelsUntilLength(posAt+1);

            //update data on the corresponding panel
            panel_DamageInfos[posAt].UpdateViewData(dt_dm, dt_dm.target);
        }
